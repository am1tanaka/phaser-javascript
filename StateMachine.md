...
ゲームの流れを考えてみます。起動すると、まずはデータを読み込む起動画面が表示されて、次にタイトル画面が表示されて、画面をクリックするとゲームが開始して・・・といったように進行することが思い浮かびます。ゲームに限らず、アプリケーションは、その時々の状況に合わせた入力方法や画面のレイアウトを切り替えながら動きます。そのような機能の違いを<b>状態</b>として定義して、プログラムの進行や操作に応じて状態を切り替えることで、規模の大きなプログラムを管理する手法があります。

Phaserも状態を管理する機能、`State Manager`を持っています。これを利用することで、状態ごとにプログラムを分割して開発することができるようになります。ここでは`State Manager`の使い方を紹介します。

## Phsaerでのシーンの構築方法
ここまで使ってきたプロジェクトは、1つの状態だけで動いていました。`preload`でファイルを読み込み、`create`でゲームワールドを構築して、`update`でフレーム毎の更新処理をして、必要であれば`render`でデバッグメッセージを表示しましたが、このようなサイクルを状態ごとにオブジェクトに定義して、ゲームに登録して、切り替えることが、`State Manager`の役割です。

Unityでは、シーン(Scene)がこれに該当します。

Phaserで状態を複数持たせるには、以下のような手順を踏みます。

1. 各状態のオブジェクトを作成して、その状態のための`preload`や`create`などのメソッドを定義する
1. 作成した状態のオブジェクトを、Phaserの開始時に全てゲームに登録
1. 最初の状態を開始(`start`)
1. 必要に応じて、`start`を呼び出して、状態を切り替える

これまで作ってきたような構造を、状態ごとに作成して、それを切り替えていくのです。
...
|ワールド削除|boolean|x|true|trueを設定すると、状態を切り替える時に、`Game.World`オブジェクトに登録されている全てのゲームオブジェクトを消して、表示リストを整理します。`Game.Stage`に追加したオブジェクトは破棄しません。それらを削除する場合は、`shutdown`メソッドなどで削除するコードを書いてください。|
|キャッシュ削除|boolean|x|false|trueを設定すると、状態を切り替える時に、preloadで読み込まれていた画像や音声をキャッシュから削除します。大きいプロジェクトで状態ごとにアセットを入れ替えたい時や、ゲームを終了する時にtrueを使います。アセットの入れ替えが不要なプロジェクトであれば、利用する必要はないでしょう。|
|パラメーター|何でもよい|x|なし|State.init関数に渡されるパラメーターです。型は何でもよく、コンマ区切りで書けば、複数のパラメーターを渡すこともできます。|

小さいプロジェクトで気にするのは、ワールド削除フラグです。それ以外はデフォルト値のままでよいでしょう。
...
#### WorldとStage
`Phaser.World`は、`this.add.image()`などで追加スプライトが追加される先のゲームの世界です。通常のスプライトなどはここに属しますので、`start()`の第2引数をtrueにすれば、状態を変更する時に削除されます。

`Phaser.Stage`は、表示されているあらゆる表示オブジェクトのルートレベルを操作します。また、ブラウザーの表示状態を監視して、フォーカスを失った時に一時停止するなどします。

表示オブジェクトは、`this.stage.addChild(表示オブジェクト);`とすることで、`Stage`に登録できます。常に画面上に表示しておきたいパラメーターなどを登録しておくと、状態を切り替える時に、ワールドのオブジェクトを削除しても残すことができます。`Stage`に所属させたオブジェクトは、`shutdown`などで、明示的に破棄する必要があります。
